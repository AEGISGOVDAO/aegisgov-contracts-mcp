...
async function getHTTPServer(price, path) {
  ...
  // x402 v2 format: accepts nested inside route config
  const routes = {
    [`POST ${path}`]: {
      accepts: {
        scheme: 'exact',
...
async function requirePayment(req, res, price) {
  ...
  try {
    ...
    // Vercel req is IncomingMessage (lacks Express's .header() method) — polyfill it
    ...
    const adapter = new ExpressAdapter(adaptedReq);
    const paymentHeader =
