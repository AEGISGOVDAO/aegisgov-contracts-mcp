 ...
 * Uses https module (no global fetch dependency).
 */
...
module.exports = async (req, res) => {
  ...
  try {
    const { status, body } = await httpsGet(`https://api.github.com/gists/${GIST_ID}`);
    if (status !== 200) {
      return res.status(502).json({ ok: false, error: `Gist read failed: ${status}` });
    }
...
function aggregate(events, server) {
  ...
  return {
    ok: true, server: 'aegisgov-contracts-mcp', asOf: new Date().toISOString(),
