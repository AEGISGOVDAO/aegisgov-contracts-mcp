/**
 * GET /metrics — funnel metrics for aegisgov-contracts-mcp
 *
 * Reads from the shared telemetry Gist and aggregates:
 *   - tool call counts (24h, 14d)
 *   - HTTP 402 / payment attempt / success counts
 *   - unique client IPs
 *   - top tools and discovery sources
 */
...
function aggregate(events, server) {
  ...
  const cutoff1d  = new Date(now); cutoff1d.setUTCHours(cutoff1d.getUTCHours() - 24);
  ...
  for (const e of mine) {
    const in14d = e.d >= cut14Str;
    const in24h = e.d === today; // rough 24h = today
  ...
  const topTools = Object.entries(toolTotals14d)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
  ...
  return {
    ...
    funnel: {
      calls: {
        last24h:    calls24h,
        last14d:    calls14d,
        byTool14d:  toolTotals14d,
        byTool24h:  toolTotals24h,
      },
      topTools,
      http402: { last14d: http402_14d, last24h: http402_24h },
      payments: {
        attempts14d:  payAtt14d,
        attempts24h:  payAtt24h,
        successes14d: payOk14d,
        successes24h: payOk24h,
        conversionRate: payAtt14d > 0
          ? `${((payOk14d / payAtt14d) * 100).toFixed(1)}%`
          : 'N/A',
      },
